...
namespace Download.NodeSystem {
    public class CoalOre : Folder, IRunnable {
        ...
        public CoalOre(Folder parent, string name, NodeCreateOptions? options = null) : base(parent, name, options) {
            RunManager = new RunManager(_disposables, RUN_OPTION);
            RunManager.RunComplete
                .Subscribe(_ => {
                    if (Parent == null) return;
                    new Coal(this.Parent.ChildRunResultTarget, CoalStatic.Instance.Name, new NodeCreateOptions { Index = GetIndex() + 1 });
                    return;
                })
                .AddTo(_disposables);
        ...
        public static new IStaticNode StaticNode => CoalOreStatic.Instance;
        ...
        public override bool OnDrop(DragContext context) {
            ...
            runManager.RunComplete
                .Subscribe(_ => {
                    new Coal(this.Parent!, Coal.StaticNode.Name, new NodeCreateOptions { Index = GetIndex() + 1 });
                })
                .AddTo(_disposables);
